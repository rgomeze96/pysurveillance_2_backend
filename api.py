...
def nlp_processing():
    if 'file' not in request.files:
        ...
        if len(search_params) == 0:
            ...
            number_of_titles = len(df_titles)
            lowered_filtered_params = []
            for filtered_parameter in filtered_params:
                filtered_parameter = filtered_parameter.lower()
                lowered_filtered_params.append(filtered_parameter)
            
            nlp_filtered_df = df[df['Title'].str.contains('|'.join(lowered_filtered_params), case=False)]
            print('Number of total results: ', number_of_titles)
            print('Number of results after NLP: ', len(nlp_filtered_df))
            first_grade_analysis = new_first_grade(nlp_filtered_df)
            second_grade_analysis = new_second_grade(nlp_filtered_df)
            third_grade_analysis = new_third_grade(nlp_filtered_df)
            print('First Grade Analysis: ', first_grade_analysis)
            print('Second Grade Analysis: ', second_grade_analysis)
            print('Third Grade Analysis: ', third_grade_analysis)
...
def new_second_grade(dataframe_for_analysis):
    df = dataframe_for_analysis
...
def new_third_grade(dataframe_for_analysis):
    df = dataframe_for_analysis
